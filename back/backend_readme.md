...
## Download → Trim → Landmark pipeline
...
- **Create a balanced dataset for experiments**: run metadata intake, download, trim, extract landmarks, and build datasets as shown below. The dataset builder uses the landmark manifest to produce reproducible train/val/test splits without touching the raw videos.
- **Train a lightweight real-time classifier**: after generating `backend/data/processed/datasets/train.npz`, run `python src/train/train_model.py --dataset backend/data/processed/datasets/train.npz --architecture tcn` (swap `lstm`/`mlp`). The script checkpoints the best accuracy for later evaluation.
- **Evaluate your best checkpoint**: point `src/train/eval_model.py` at the test split and checkpoint to view accuracy/confusion matrix. This helps you triage failing classes before export.
- **Export for mobile**: once you have a checkpoint, first call `python src/export/export_onnx.py --checkpoint backend/models/checkpoints/<best>.pt`, then `python src/export/export_tflite.py --onnx backend/models/exported/classifier.onnx --quantize`.
...
- `python src/data/build_dataset.py backend/data/processed/landmarks/manifest.json` — build train/val/test `.npz` dataset files from `[T,21,3]` sequences. Flags: `--splits`, `--max-samples`, `--dataset-dir`.
- `python src/train/train_model.py --dataset backend/data/processed/datasets/train.npz` — train MLP/LSTM/TCN models, save checkpoints to `backend/models/checkpoints/`. Flags: `--architecture`, `--hidden-dim`, `--dropout`, `--batch-size`, `--epochs`, `--lr`, `--device`.
- `python src/train/eval_model.py --dataset backend/data/processed/datasets/test.npz --checkpoint backend/models/checkpoints/<best>.pt` — report accuracy/confusion matrix. Flags: `--architecture`, `--hidden-dim`, `--dropout`, `--device`.
- `python src/export/export_onnx.py --checkpoint backend/models/checkpoints/<best>.pt` — export ONNX with metadata-aware shapes. Flags: `--architecture`, `--sequence-length`, `--feature-dim`, `--hidden-dim`.
